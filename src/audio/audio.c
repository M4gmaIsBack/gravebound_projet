...
void configurerVolume(AudioAssets *audio, int volume) {
    if (volume < 0) volume = 0; // Volume minimum
    if (volume > MIX_MAX_VOLUME) volume = MIX_MAX_VOLUME; // Volume maximum
...
void jouerSon(Mix_Chunk *sound) {
    if (sound) {
        Mix_PlayChannel(-1, sound, 0); // Lecture sur le premier canal libre
    } else {
